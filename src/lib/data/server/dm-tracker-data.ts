...
export async function getDMTrackerData(): Promise<Result<DMTrackerData, Error>> {
  ...
  return cache.get(
    ...
    async () => {
      ...
      // Try the pre-built JSON snapshot first — same pattern as ExcelAdapter.
      // Vercel's serverless filesystem has no Python/openpyxl, so the live
      // extraction script below only ever works in local dev; production
      // must read the snapshot generated by `npm run refresh-data`.
      const snapshotPath = join(projectRoot, 'src', 'data', 'dm-tracker-snapshot.json')
      try {
        const raw = readFileSync(snapshotPath, 'utf-8')
        const dmData: DMTrackerData = JSON.parse(raw)
        console.log(
          `[getDMTrackerData] Loaded from snapshot — consolidated DM%: ${dmData.consolidated.dm_pct.toFixed(2)}%`
        )
        return ok(dmData)
      } catch (snapshotError) {
        if ((snapshotError as NodeJS.ErrnoException).code !== 'ENOENT') {
          return err(
            new Error(
              `Failed to load DM tracker snapshot at "${snapshotPath}": ${(snapshotError as Error).message}. ` +
              `Fix the underlying issue (e.g. run \`npm run refresh-data\` to regenerate the snapshot) and retry.`
            )
          )
        }
        console.warn('[getDMTrackerData] Snapshot not found, falling back to Python bridge (local dev only)...')
      }
      ...
      try {
        ...
        // Last resort: hardcoded static snapshot from 2026-03-09. Should only
        // ever be hit if BOTH the pre-built JSON snapshot is missing AND the
        // local Python bridge fails — i.e. this should never fire in
        // production now that the snapshot above is the primary path.
        console.warn('[DMTracker] Python parsing unavailable and no snapshot found — serving static fallback from 2026-03-09. Data may be stale.')
