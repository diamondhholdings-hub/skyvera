...
- **DM Tracker snapshot pattern** — `getDMTrackerData()` reads a pre-built `src/data/dm-tracker-snapshot.json` first (regenerated by `npm run refresh-data`), same as `ExcelAdapter`; only falls back to the live Python bridge in local dev, and to a hardcoded static snapshot as a last resort. Python/openpyxl doesn't exist on Vercel serverless, so anything that shells out to `python3` at request time will silently degrade in production unless it follows this pre-built-snapshot pattern
- **Rate limiter usage** — `rateLimit()` is synchronous (not async, despite older JSDoc examples showing `await`): `const rl = rateLimit(request, {limit, window}); if (!rl.success) return NextResponse.json({error:'Too many requests', retryAfter: rl.reset}, {status:429, headers: rateLimitHeaders(rl)})`. IP extraction prefers `request.ip`/`x-real-ip` (Vercel-set, not client-controllable) over `X-Forwarded-For`, and if it must fall back to `X-Forwarded-For`, uses the LAST entry (appended by Vercel's own edge) not the first (attacker-controllable)
- **SOQL/SQL escaping** — always escape backslashes BEFORE quotes (`str.replace(/\\/g,'\\\\').replace(/'/g,"\\'")`) — quotes-only escaping is bypassable when input contains a backslash immediately before a quote
- **Inline styles beat Tailwind classes** — an inline `style={{ display: 'flex' }}` always overrides `className="hidden lg:flex"` regardless of viewport/breakpoint. If a component needs a class like `hidden` to control visibility, don't also set that CSS property inline
- **Mobile horizontal overflow** — any raw `<table>` or fixed-width flex row needs its own `overflow-x: auto` wrapper, or it forces the whole page to scroll sideways on narrow viewports. `html`/`body` also carry `overflow-x: hidden` in `globals.css` as a backstop
...
> **Note:** These figures reflect the Q3'26 budget workbook as of 2026-08-05. This section will go
> stale again next quarter — re-derive from the live dashboard or `src/data/skyvera-snapshot.json`
> rather than trusting these numbers indefinitely. Real financial metrics are wired end-to-end now
> (see the CHANGELOG [1.5.0] entry) — these are no longer hardcoded benchmarks, they're what the
> current workbook actually says.

### Key Financial Issues (Q3'26)
1. **Margin Gap: -$199K** (-1.6 percentage points below the 63.0% blended target)
2. **RR Declining -$791K vs. Prior Plan** - Cloudsense -$168K, Kandy -$531K, STL -$30K
3. **AR > 90 Days: $9.8M** - 21.7% of ARR — collection/churn risk
4. **YoY Revenue Change: -20.4%** - Rule of 40 = 41.0% (still passing, but a rich company-wide
   figure only covers the 3 core BUs' historical comparison sheets, ~86% of total revenue)
5. **Salesforce UK Contract: $4.1M/year** - Largest vendor cost (last verified against Q1'26 data;
   not yet re-extracted from Vendor Pivots for Q3'26 — treat as a known fact about the contract,
   not a live-computed figure)

### Business Unit Margins (Q3'26, real per-BU Margin Target from the Excel P&L sheets)
- **Cloudsense**: 59.6% net margin (target: 60%)
- **Kandy**: 59.2% net margin (target: 60%)
- **STL**: 55.9% net margin (target: 75%)
...
3. **Compare current-quarter Plan vs Prior Plan** columns to identify variances (column headers are named per-quarter, e.g. "Q3'26 BU Plan" / "Q3'26 Prior BU Plan" — the label changes every quarter, the pattern doesn't)
...
### Open Issues (as of 2026-08-05)

None — 0 open beads issues. Run `bd ready` to confirm current state before assuming this is still true.
