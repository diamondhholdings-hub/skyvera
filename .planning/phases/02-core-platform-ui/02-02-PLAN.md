...
must_haves:
  ...
  artifacts:
    - path: "src/app/dashboard/page.tsx"
      provides: "Dashboard Server Component with Suspense boundaries for streaming, includes server wrapper logic for chart data"
      min_lines: 30
  ...
  key_links:
    - from: "src/app/dashboard/page.tsx"
      ...
      via: "Direct import and call in Server Component -- getRevenueTrendData() and getBUSummaries() called server-side"
      pattern: "getDashboardData|getBUSummaries|getRevenueTrendData"
...
<objective>
Build the financial KPI dashboard page showing multi-BU breakdown with current vs target metrics, revenue trend charts, and margin comparisons. This plan satisfies requirement DASH-01 (Enhanced financial KPI dashboard with multi-BU breakdown) ONLY. Requirement DASH-02 (Proactive alerts dashboard) is satisfied by Plan 02-03.
...
<task type="auto">
  ...
  <action>
    1. Create `src/app/dashboard/page.tsx` as the main dashboard Server Component:
       ...
       - NOTE: This file also serves as the server wrapper for chart components (Task 2). It will contain async server wrapper components that call getRevenueTrendData() and getBUSummaries() server-side and pass data as props to client chart components. This keeps all data fetching server-side per 02-RESEARCH.md anti-pattern guidance.
    ...
    4. Create `src/app/dashboard/components/recent-alerts-preview.tsx` as async Server Component:
       ...
       - NOTE: The /alerts route page is built by Plan 02-03 (also Wave 2). During parallel execution, this link may briefly 404 but the route's loading.tsx skeleton already exists from Plan 02-01. This is acceptable -- the link is correct and will resolve once Plan 02-03 completes.
...
<task type="auto">
  ...
  <action>
    ...
    2. Update `src/app/dashboard/page.tsx` to include async server wrapper components that:
       - Call getRevenueTrendData() server-side and pass result as props to RevenueChart client component
       - Call getBUSummaries() server-side and pass result as props to MarginComparison client component
       - These wrappers are defined inside page.tsx (or as named async functions) to keep data fetching server-side (02-RESEARCH.md anti-pattern: no client-side API calls for initial data)
       - This is the server wrapper pattern: async Server Component fetches data, passes to "use client" chart component as props
    ...
    4. Finalize dashboard/page.tsx layout to include both chart components:
  ...
  <verify>
    ...
    Verify server-side data fetching: check that getRevenueTrendData() and getBUSummaries() are called in Server Component code (grep page.tsx for these function calls -- they must appear outside any "use client" boundary). No client-side fetch() or useEffect() calls for chart data.
  </verify>
  <done>
    ...
    Charts receive data from Server Components (no client-side fetching) -- getRevenueTrendData() and getBUSummaries() are called server-side in page.tsx wrapper components.
...
10. Server-side data fetching confirmed: getRevenueTrendData() and getBUSummaries() called in Server Component, not via client-side fetch
</verification>
...
- All data sourced from Phase 1 Excel adapter via server data functions (server-side only, no client API calls)
- Requirement DASH-01 (Enhanced financial KPI dashboard with multi-BU breakdown) satisfied
- NOTE: DASH-02 is NOT covered by this plan -- it is addressed by Plan 02-03
</success_criteria>
