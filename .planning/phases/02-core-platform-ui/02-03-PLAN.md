...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/app/alerts/page.tsx"
      provides: "Alerts dashboard showing at-risk accounts and metric anomalies (satisfies DASH-02)"
      min_lines: 20
...
<objective>
Build the customer account directory with search, filtering, and health scoring, plus the proactive alerts dashboard showing at-risk accounts and metric anomalies. This plan satisfies requirements ACCT-01, ACCT-02, ACCT-03 (account directory and health scoring) AND DASH-02 (proactive alerts dashboard). Plan 02-02 covers DASH-01 only; this plan covers the remaining Phase 2 requirements.
...
# Plan 01 SUMMARY needed for component interfaces and CustomerWithHealth type (which includes `bu` field)
...
<task type="auto">
  ...
  <action>
    1. Create `src/app/accounts/page.tsx` as async Server Component:
       ...
       - NOTE: CustomerWithHealth now includes a `bu` field (added in Plan 02-01 Task 2). The account-table.tsx receives this field directly -- no additional type extension needed here.
    ...
    4. Create `src/app/accounts/components/account-table.tsx` as Client Component ("use client"):
       - Props: customers (CustomerWithHealth[]) -- this type includes `bu: string` field from Plan 02-01
       ...
       - Columns:
         ...
         b) Business Unit: sortable, filterable, renders as Badge component -- accesses `row.original.bu`
  ...
  <verify>
    ...
    Verify BU column renders correctly (confirms `bu` field exists on CustomerWithHealth).
  </verify>
  <done>
    ...
    BU column renders Badge with business unit name from `bu` field.
...
8. BU column renders correctly with Badge (confirms bu field on CustomerWithHealth)
9. /alerts shows severity-sorted alert cards with summary
10. Red alerts appear before yellow alerts
11. Alert cards show account names, metrics, and relative timestamps
12. Both pages show data freshness timestamps
</verification>
...
- Requirements ACCT-01, ACCT-02, ACCT-03 satisfied (account directory and health)
- Requirement DASH-02 (Proactive alerts dashboard) satisfied (Plan 02-02 covers DASH-01 only)
</success_criteria>
